...
if [ -z "$CUSTOM_PACKAGES" ]; then
  ...
  git clone --depth=1 https://github.com/hza81007155/istoreos-app.git /tmp/store-run-repo

  # 拷贝 run/x86 下所有 run 文件和ipk文件 到 extra-packages 目录
  mkdir -p extra-packages
  cp -r /tmp/store-run-repo/run/x86/* extra-packages/
  cp -f /x86/banner package/base-files/files/etc/
...
# 开始构建
make image PROFILE=generic PACKAGES="$PACKAGES" FILES="files" ROOTFS_PARTSIZE=256
